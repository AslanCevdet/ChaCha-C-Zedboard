...
void ChaCha20::scan(char* buffer, uint32_t max_size){
	 ...
	 uint8_t received_byte;
	    ...
	    while(1)
	    {
	        if (XUartPs_IsReceiveData(XPAR_PS7_UART_1_BASEADDR)) // Using UART1
	        {
	            received_byte = XUartPs_ReadReg(XPAR_PS7_UART_1_BASEADDR, XUARTPS_FIFO_OFFSET);
	            if (received_byte == '\n' || received_byte == '\r')
	            {
	                break;
	            }
